...
class App(object):
    ...
    def _build_turn(self, parent):
        ...
        # ★ 다시 5초 -> 10초 (2026-09-24). **안 나온 상대 벤치를 세기 시작하면서**
        #   한 판이 3배로 무거워졌다 (상대가 1마리에서 3마리가 된다). 같은 자리를
        #   씨앗 셋으로 재 봤다 (내 6마리 vs 한카리아스 + 숨은 벤치 2) —
        #     5초 : 더시마사리로 교체 / 하품 / 타부자고로 교체  (오차 ±50 — 한 판만 돌린 후보)
        #    10초 : 스텔스록 / 하품 / 하품                     (오차 ±3.4)
        #    25초 : 스텔스록 / 하품 / 하품                     (오차 ±2.1)
        #   **5초는 답이 판마다 달라진다.** 10초와 25초는 같은 두 수 안에서만 흔들린다.
