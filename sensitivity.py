...
FORM_ALTERNATIVES = [
    ...
    ("nature_mismatch", 0.05, "성격↔형태: 표본 0.001 → 내 감 0.05 로 되돌림"),
    ...
    # 메가 세기는 가능도 곡선이 평평하다. 그래서 **데이터가 받쳐 주는 범위
    # 안에서** 흔들어 본다 — 3.0 같은 값은 가능도가 10.9 나빠서 애초에 후보가
    # 아니고, 그걸로 추천이 바뀌어도 "답이 흔들린다" 고 읽으면 안 된다.
    ("mega_stat_strength", 0.375, "메가 세기 0.75 → 0.375 (밴드 하단)"),
    ("mega_stat_strength", 1.125, "메가 세기 0.75 → 1.125 (밴드 상단)"),
    ("mega_stat_strength", 1.5, "메가 세기 0.75 → 1.5 (내 감이었던 값)"),
    ("mega_stat_strength", 0.0, "메가 종족값을 안 쓴다 (밴드 밖 — 참고용)"),
]
