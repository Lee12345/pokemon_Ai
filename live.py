...
SWITCH_NAME = "교체(뺀다)"      # search.SWITCH_KEY 와 같은 말 (창은 search 를 안 부른다)


def opp_guess_lines(rows, opp_name=None, mega=0.0):
    """「상대는 이번 턴에 무엇을 할까」 를 창에 띄울 줄로. rows 는 [(이름, 비율)].

    ★ 사용자가 짚었다 (2026-09-23): *"교체를 해야할지 상대 교체를 예측해야할지
      기타등등.."* — 내 수만 알려 주고 **상대가 무엇을 할지는 한마디도 안 했다.**

    ★ **잰 것이 아니다.** 돌려 본 판마다 상대 세트를 새로 뽑고, 그 세트로 나를 제일
      아프게 하는 수를 고른 것이다. 상대 사람의 버릇이 아니다. 그러니 '제일 아픈 수'
      라고만 말한다.
    ★ **이번 턴에 상대가 빼는 것도 이제 센다** (2026-09-24). 사용자가 물었다 —
      *"교체로 빠지는건 왜 고려X?"* 둘째 턴부터는 늘 보고 있었는데 **첫 턴만**
      계획으로 굳어 있었다. 지금은 둘째 턴과 **같은 규칙**(`Battle.should_switch`,
      실제로 잰 1대1 승률)으로 첫 턴도 본다.
    ! **그 판단은 0% 아니면 100% 로 쏠린다.** 규칙이 정해진 것이라, 같은 대면이면
      돌려 본 판마다 같은 답이 나온다 (상대 세트가 갈릴 때만 흔들린다).
      그러니 「교체 100%」 는 *상대가 반드시 뺀다* 가 아니라
      **"내 계산으로는 빼는 것이 상대에게 낫다"** 로 읽어야 한다.
      잰 것 (하마돈 vs 한카리아스+패리퍼+블래키, 6초): 안 세면 하품·교체·지진·스텔스록이
      0.405~0.419 로 **구별이 안 됐는데**, 세니 하품 0.266 / 스텔스록 0.163 으로
      1등이 뚜렷해졌다. 점수가 전체적으로 내려간 것은 상대가 나쁜 대면을 피하기 때문이다.
    """
    if not rows:
        return []
    who = "상대 %s" % opp_name if opp_name else "상대"
    swap = next((s for n, s in rows if n == SWITCH_NAME), 0.0)
    moves = [(n, s) for n, s in rows if n != SWITCH_NAME]
    out = []
    if moves:
        part = " · ".join("「%s」 %.0f%%" % (n, s * 100) for n, s in moves[:2])
        out.append("◆ %s 가 고를 것 같은 수: %s" % (who, part))
    # 메가는 기술과 따로 센다 — 안 그러면 「메가+대검돌격 39%」 와 「대검돌격 37%」 로
    # 갈려서, 같은 기술을 76% 로 노린다는 사실이 안 보인다.
    if mega >= 0.10:
        out.append("◆ 상대가 이번 턴에 메가진화까지 할 수 있습니다 (돌려 본 판의 %.0f%%)"
                   % (mega * 100))
    if swap >= 0.10:
        out.append("◆ **상대가 이번 턴에 뺄 자리입니다** — 돌려 본 판의 %.0f%% 에서 뺐습니다"
                   " (내 계산으로 그게 상대에게 낫다는 뜻입니다)" % (swap * 100))
    return out
