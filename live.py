...
def opp_guess_lines(rows, opp_name=None, mega=0.0):
    ...
    ★ **잰 것이 아니다.** 돌려 본 판마다 상대 세트를 새로 뽑고, 그 세트로 나를 제일
      ...
      라고만 말한다.
    ★ **이번 턴에 상대가 교체하는 경우는 안 세어 봤다** — 상대의 첫 턴은 이 수로
      굳혀 놓고 돌리기 때문이다. 이 말을 반드시 같이 적는다. 안 적으면 사용자는
      "교체까지 따져 봤구나" 로 읽는다 — 조용히 틀리는 자리다.
    """
    if not rows:
        return []
    ...
    if mega >= 0.10:
        out.append("◆ 상대가 이번 턴에 메가진화까지 할 수 있습니다 (돌려 본 판의 %.0f%%)"
                   % (mega * 100))
    out.append("· 이번 턴에 상대가 교체하는 경우는 안 셉니다 — 그건 직접 보셔야 합니다")
    return out
