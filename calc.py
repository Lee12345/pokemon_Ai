...
CONFIG = {
    ...
    # 격자를 넓혀 다시 재 보니 **0.001 이 진짜 최댓값**이었다. 더 내리면
    # 나빠진다 (1e-5 는 3.7, 1e-7 은 8.3 만큼 가능도가 떨어진다). 즉 반례가
    # 아주 없는 것이 아니라 **0.1% 쯤은 있다.** 0.05(내 감)는 15.5 만큼
    # 나빠서 확실히 틀렸다. 처음에 '격자 끝' 으로 보인 것은 내 격자의
    # 제일 낮은 값이 우연히 정답과 같았기 때문이다.
    "nature_mismatch": 0.001,       # 표본으로 맞춤 (내 감 0.05, 그 전 0.01)
    ...
    # 배분 없는 개체를 걸러낸 뒤 다시 재니 **0.75** 가 나왔다 (그전엔 1.5).
    # 다만 **곡선이 평평하다** — 0.375~1.125 가 가능도 차이 0.74 안에 다 들어온다.
    # 표본 1291마리에서 그 정도 차이는 구별이 안 된다. 데이터가 말해 주는 것은
    # "0.4~1.5 사이이고, 0 도 3 도 아니다" 까지다 (0 은 2.3, 3 은 10.9 나쁘다).
    # 랭커 표본이라는 점을 감안하면 실제 래더는 더 낮을 쪽이므로 0.75 를 쓴다.
    #
    # ! 밴드 안(1.125, 1.5)에서도 추천이 4개 중 1개 바뀐다. 다만 그때
    #   승률 흔들림이 1.3%p 뿐이다 — 두 계획이 거의 동점이라 어느 쪽을
    #   골라도 손해가 없다는 뜻이다. 성격 쪽은 뒤집힐 때 5~9%p 라서 다르다.
    "mega_stat_strength": 0.75,     # 표본으로 맞춤 (내 감 1.5)
}
