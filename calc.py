...
CONFIG = {
    ...
    # 사용률에는 조합 정보가 없어서 **이 값 자체는 데이터로 확인이 안 된다.**
    # 그래서 sensitivity.py 에 넣어 두고 답이 흔들리는지 따로 잰다. forms.py 참고.
    "form_mismatch": 0.15,
    # 도구가 어느 형태에 쏠리는지는 **데이터에서 잰다** (forms.item_tendency).
    # 여기 있는 건 그 상관을 승산으로 바꿀 때의 세기다 — 미확인.
    #   승산 배율 = exp(세기 x 상관).  상관 0.69 에 세기 1.0 이면 약 2배.
    # 0 으로 두면 도구를 형태 추론에 안 쓰는 것과 같다.
    "item_tendency_strength": 1.0,
    # 성격은 기술보다 규칙이 딱딱하다. CS형이 지진을 커버로 드는 일은 실제로
    # 있지만, CS형이 특수공격을 깎는 성격을 드는 일은 사실상 없다 — 그냥 손해다.
    # 그래서 기술(0.15)보다 세게 깎는다. 0 이 맞을 수도 있지만, 확률을 0 으로
    # 만들면 관찰이 어긋났을 때 빠져나갈 곳이 없어지므로 남겨 둔다. 미확인.
    "nature_mismatch": 0.05,
